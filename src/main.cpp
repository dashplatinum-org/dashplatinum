...
#if defined(NDEBUG)
#error "DASHP cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "DASHP Signed Message:\n";
...
int64_t GetBlockValue(int nHeight)
{
    ...
    if (nHeight+1 == 2) {
      nSubsidy = 294000 * COIN; // Money supply premine for the swap pre-venda
    } else if (nHeight+1 == 3) {
      nSubsidy = 100000 * COIN; // staking wallets and masternodes to start the new chain
    } else if (nHeight+1 >= 3 && nHeight+1 < 14400) {
      nSubsidy = 1.10 * COIN;
      ...
      nSubsidy = 2.90 * COIN;
    } else if (nHeight+1 >= 57600 && nHeight+1 < 172800) {
      nSubsidy = 5.10 * COIN;
    } else if (nHeight+1 >= 172800 && nHeight+1 < 345600) { //2y
      nSubsidy = 4.59 * COIN;
    } else if (nHeight+1 >= 345600 && nHeight+1 < 518400) { //3y
      nSubsidy = 4.05 * COIN;
    } else if (nHeight+1 >= 518400 && nHeight+1 < 604800) { //4Y
      nSubsidy = 3.64 * COIN;
    } else if (nHeight+1 >= 604800) {
      nSubsidy = 3.28 * COIN;
    }
...
{
    RenameThread("dashplatinum-scriptch");
...
int ActiveProtocol()
{
    if (chainActive.Height() < 122862) {
      ...
      return 72912; // Budget system enabled
    }
}
